...
export default function AdminR2O() {
  ...
  // Invoice + linked POS table (e.g. Delivery 1)
  ...
  async function saveSalesMode() {
    ...
    try {
      ...
      const { data } = await api.put('/r2o/sales-mode', {
        ...
        tableName: String(selected?.table_name || ''),
      });
      setStatus((s) => ({ ...s, salesMode: 'invoice', tableId: data.tableId, tableName: data.tableName }));
      toast.success(`Rechnung wird mit Tisch „${data.tableName || data.tableId}“ verknüpft`);
    } catch (e) {
  ...
  return (
    <div className="w-full max-w-3xl mx-auto px-4 sm:px-6 py-6 space-y-6">
      ...
      {status?.configured && (
        <StepCard
          step={3}
          title="Rechnung + Tisch"
          description="Beim Akzeptieren wird sofort eine Rechnung in ready2order erstellt (sichtbar unter Rechnungen) und mit dem gewählten Tisch verknüpft."
        >
          <div className="space-y-3">
            ...
            {status?.tableName && (
              <p className="text-xs text-emerald-400/90">
                Aktuell verknüpft: {status.tableName}
              </p>
            )}
            <button type="button" onClick={saveSalesMode} disabled={savingMode || !tableId} className="btn-primary text-sm disabled:opacity-40">
              {savingMode ? 'Speichere…' : 'Tisch speichern'}
            </button>
          </div>
        </StepCard>
      )}

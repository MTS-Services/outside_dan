  ...
  return (
    <div className="p-6 space-y-4">
      <div className="flex justify-between items-center flex-wrap gap-3">
        <h1 className="font-display text-3xl">Kunden ({rows.length})</h1>
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Suchen…" className="input h-10 w-64" />
      </div>
          ...
          {filtered.map((u) => (
            <div key={u.id} className="p-4 flex items-center gap-4">
              ...
              <button disabled={busyId === u.id} onClick={() => toggleBlock(u)}
                className={`px-3 py-1.5 rounded-lg text-xs font-semibold disabled:opacity-40 ${u.blocked ? 'bg-emerald-500/20 text-emerald-400' : 'bg-red-500/15 text-red-400'}`}>
